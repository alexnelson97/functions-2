...
console.log(printParameters(2, 3));
////////// PROBLEM 3 //////////
/*
...
greeting("Alex", "Nelson", displayGreeting);
...
const pricesPlusTax = (arr, cb) => {
  const totalCostArr = [];
  for (let i = 0; i < arr.length; i++) {
    const totalCost = arr[i] * 1.2;
    totalCostArr.push(totalCost);
  }
  cb(totalCostArr);
};

function printTotalCost(totalCostArr) {
  console.log(`The new array plus tax = [${totalCostArr}]`);
}

pricesPlusTax(prices, (totalCostArr) => {
  console.log(`The new array plus tax = [${totalCostArr}]`);
});
...
const multiplyingFactory = (firstNumber) => {
